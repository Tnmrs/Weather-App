...
extension View {
    func innerShadow<S: Shape, SS: ShapeStyle>(shape: S, color: SS, lineWidht: CGFloat = 0, offsetX: CGFloat = 0, offsetY: CGFloat = 0, blur:  CGFloat = 4, blendMode: BlendMode = .normal, opacity: Double = 1) -> some View {
        return self
            .overlay {
               shape
                    .stroke(color, lineWidth: lineWidht) //Размытие цвета
                    .blendMode(blendMode) //наложения цветов поверх друга
